...
if (modificarCategoriasModal) {
    ...
    const id_categoria = button.getAttribute('data-bs-id-categoria')

    // If necessary, you could initiate an Ajax request here
    // and then do the updating in a callback.

    // Update the modal's content.
    const modal_nombre = modificarCategoriasModal.querySelector('.modal-body input[name="nombre"]')
    const modal_sueldo_base = modificarCategoriasModal.querySelector('.modal-body input[name="sueldo_base"]')
    const modal_departamento = modificarCategoriasModal.querySelector('.modal-body #id_departamento')
    const modal_id_categoria = modificarCategoriasModal.querySelector('.modal-body #id_categoria')
